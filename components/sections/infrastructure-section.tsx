"use client";

import { motion, useReducedMotion } from "framer-motion";
import { ShieldCheck } from "lucide-react";
import { useState } from "react";
import { RevealText } from "@/components/ui/reveal-text";
import { dropIn, dropInReduced, imageReveal } from "@/lib/motion";
import { getIcon } from "@/lib/icons";
import type { ExperienceItem, InfrastructureContent, TechnicalFeature } from "@/lib/content-schema";

function ExperienceCard({ experience }: { experience: ExperienceItem }) {
  const [imageUnavailable, setImageUnavailable] = useState(false);
  const Icon = getIcon(experience.icon);

  return (
    <article className="group relative min-h-72 aspect-[4/3] overflow-hidden rounded-3xl border border-stone bg-stone-soft">
      {experience.image && !imageUnavailable && (
        <motion.img
          variants={imageReveal}
          src={experience.image}
          alt={experience.title}
          loading="lazy"
          decoding="async"
          className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]"
          onError={() => setImageUnavailable(true)}
        />
      )}
      <div className={`absolute inset-0 ${!experience.image || imageUnavailable ? "bg-[radial-gradient(circle_at_85%_10%,rgba(213,39,44,0.18),transparent_36%),linear-gradient(135deg,var(--stone-soft),var(--paper))]" : "bg-gradient-to-t from-[#18191a]/90 via-[#18191a]/20 to-transparent"}`} />

      <div className="relative flex h-full flex-col justify-end p-6 tablet:p-7">
        {(!experience.image || imageUnavailable) && (
          <span className="mb-auto flex h-11 w-11 items-center justify-center rounded-xl bg-paper text-red shadow-card">
            <Icon className="h-5 w-5" aria-hidden="true" />
          </span>
        )}
        <div className={!experience.image || imageUnavailable ? "text-ink" : "text-white"}>
          {(!experience.image || imageUnavailable) && (
            <p className="mb-2 font-display text-[9px] font-bold uppercase tracking-[0.15em] text-graphite-soft">Render em breve</p>
          )}
          <h3 className="font-display text-xl font-extrabold uppercase tracking-[-0.035em]">{experience.title}</h3>
          <p className={`mt-2 max-w-md text-sm leading-6 ${!experience.image || imageUnavailable ? "text-ink-soft" : "text-white/80"}`}>{experience.description}</p>
        </div>
      </div>
    </article>
  );
}

function TechnicalFeatureCard({ feature }: { feature: TechnicalFeature }) {
  const Icon = getIcon(feature.icon);
  return (
    <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-stone-soft text-red">
      <Icon className="h-4 w-4" aria-hidden="true" />
    </span>
  );
}

export function InfrastructureSection({ content }: { content: InfrastructureContent }) {
  const shouldReduceMotion = useReducedMotion();

  return (
    <section id="infraestrutura" className="scroll-mt-24 bg-paper py-20 mobile:py-24 tablet:py-28">
      <div className="mx-auto max-w-site px-5 mobile:px-7 tablet:px-6">
        <motion.div
          initial={shouldReduceMotion ? false : { opacity: 0, y: 56 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.25 }}
          transition={{ duration: shouldReduceMotion ? 0 : 0.7, ease: [0.22, 1, 0.36, 1] }}
          className="grid gap-6 tablet:grid-cols-[minmax(0,1fr)_minmax(16rem,0.55fr)] tablet:items-end tablet:gap-12"
        >
          <div>
            <p className="font-display text-[10px] font-bold uppercase tracking-[0.16em] text-red">{content.eyebrow}</p>
            <h2 className="mt-4 max-w-3xl font-display text-[clamp(2rem,4.5vw,3.25rem)] font-extrabold uppercase leading-[0.98] tracking-[-0.045em] text-ink">
              <RevealText text={content.heading} />
            </h2>
          </div>
          <p className="max-w-md text-base leading-7 text-ink-soft">{content.paragraph}</p>
        </motion.div>

        <motion.div
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.18 }}
          variants={{ hidden: {}, visible: { transition: { staggerChildren: shouldReduceMotion ? 0 : 0.16 } } }}
          className="mt-14 grid mobile:mt-16 tablet:mt-20 tablet:grid-cols-3"
          style={{ gap: "clamp(2rem, 3vw, 3rem)" }}
        >
          {content.experiences.map((experience) => (
            <motion.div
              key={experience.title}
              variants={shouldReduceMotion ? dropInReduced : dropIn}
              whileHover={shouldReduceMotion ? undefined : { y: -12, scale: 1.02, rotateX: -1.5 }}
              whileTap={shouldReduceMotion ? undefined : { scale: 0.985 }}
              className="h-full transform-gpu [perspective:1000px]"
            >
              <ExperienceCard experience={experience} />
            </motion.div>
          ))}
        </motion.div>

        <div className="mt-14 border-t border-stone pt-10 tablet:mt-20 tablet:pt-12">
          <div className="flex max-w-2xl flex-col gap-3">
            <p className="font-display text-[10px] font-bold uppercase tracking-[0.16em] text-red">{content.technicalEyebrow}</p>
            <h3 className="font-display text-[clamp(1.55rem,3vw,2.25rem)] font-extrabold uppercase leading-[1.03] tracking-[-0.04em] text-ink">
              <RevealText text={content.technicalHeading} />
            </h3>
          </div>

          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.16 }}
            variants={{ hidden: {}, visible: { transition: { staggerChildren: shouldReduceMotion ? 0 : 0.1 } } }}
            className="mt-8 grid gap-3 mobile:grid-cols-2 tablet:grid-cols-3 tablet:gap-4"
          >
            {content.technicalFeatures.map((feature) => (
              <motion.article
                key={feature.title}
                variants={shouldReduceMotion ? dropInReduced : dropIn}
                whileHover={shouldReduceMotion ? undefined : { y: -9, scale: 1.015, boxShadow: "0 24px 60px rgba(27, 27, 26, 0.12)" }}
                whileTap={shouldReduceMotion ? undefined : { scale: 0.985 }}
                className="transform-gpu rounded-2xl border border-stone bg-surface p-5"
              >
                <TechnicalFeatureCard feature={feature} />
                <h4 className="mt-5 font-display text-sm font-extrabold uppercase tracking-[-0.02em] text-ink">{feature.title}</h4>
                <p className="mt-2 text-sm leading-6 text-ink-soft">{feature.description}</p>
              </motion.article>
            ))}
          </motion.div>
        </div>

        <motion.div
          initial={shouldReduceMotion ? false : { opacity: 0, y: 12 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.5 }}
          transition={{ duration: shouldReduceMotion ? 0 : 0.36, ease: "easeOut" }}
          className="mt-8 flex flex-col gap-4 rounded-2xl border border-stone bg-surface p-5 mobile:flex-row mobile:items-center mobile:gap-5 tablet:mt-10 tablet:p-6"
        >
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-red text-cta-primary-text">
            <ShieldCheck className="h-5 w-5" aria-hidden="true" />
          </span>
          <div>
            <h3 className="font-display text-sm font-extrabold uppercase tracking-[-0.02em] text-ink">{content.bannerTitle}</h3>
            <p className="mt-1 text-sm leading-6 text-ink-soft">{content.bannerDescription}</p>
          </div>
        </motion.div>
      </div>
    </section>
  );
}
